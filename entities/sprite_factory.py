...
def make_player_frames(state, direction='front'):
    ...
    import os as _os
    sprite_dir = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
                               'assets', 'sprites')
    ...
    if _os.path.isdir(sprite_dir):
        frames = []
        if state == 'idle':
            path = _os.path.join(sprite_dir, f'marine_{direction}_idle_48.png')
            if _os.path.exists(path):
                surf = pygame.image.load(path).convert_alpha()
                frames.append(surf)
                frames.append(surf.copy())  # 2 idle frames
        elif state == 'walk':
            for i in range(2):
                path = _os.path.join(sprite_dir, f'marine_{direction}_walk{i}_48.png')
                if _os.path.exists(path):
                    frames.append(pygame.image.load(path).convert_alpha())
            # Pad to 4 frames by cycling
            while len(frames) < 4:
            ...
            path = _os.path.join(sprite_dir, f'marine_{direction}_shoot_48.png')
            if _os.path.exists(path):
                surf = pygame.image.load(path).convert_alpha()
                frames.append(surf)
                # Frame 2 = idle (back to normal)
                idle_path = _os.path.join(sprite_dir, f'marine_{direction}_idle_48.png')
                if _os.path.exists(idle_path):
                    frames.append(pygame.image.load(idle_path).convert_alpha())
                else:
                    frames.append(surf.copy())
        elif state == 'reload':
            # Use idle frames for reload
            path = _os.path.join(sprite_dir, f'marine_{direction}_idle_48.png')
            if _os.path.exists(path):
                surf = pygame.image.load(path).convert_alpha()
                for _ in range(3):
            ...
            path = _os.path.join(sprite_dir, f'marine_{direction}_hurt_48.png')
            if _os.path.exists(path):
                frames.append(pygame.image.load(path).convert_alpha())
        elif state == 'dead':
            path = _os.path.join(sprite_dir, f'marine_{direction}_dead_48.png')
            if _os.path.exists(path):
                surf = pygame.image.load(path).convert_alpha()
                for _ in range(3):
                    frames.append(surf.copy())
